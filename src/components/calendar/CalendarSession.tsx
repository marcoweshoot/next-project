...
const CalendarSession: React.FC<CalendarSessionProps> = ({ session, isLast }) => {
  ...
  const getExperienceLabel = (experience?: string) => {
    switch (experience?.toLowerCase()) {
      ...
      default: return 'Tutti i livelli';
    }
  };
  ...
  return (
    <div className={`flex flex-col md:flex-row md:items-center p-4 md:p-6 ${!isLast ? 'border-b border-gray-100' : ''} hover:bg-gray-50 transition-colors`}>
      ...
      <div className="flex-1 min-w-0 mb-4 md:mb-0">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-600 mb-2">
          <div className="flex items-center gap-2">
            <img
              src={session.tour.coach.avatar?.url || 'https://wxoodcdxscxazjkoqhsg.supabase.co/storage/v1/object/public/picture//Coach-WeShoot.avif'}
      ...
      <div className="flex-shrink-0 flex flex-row md:flex-col items-center md:items-end justify-between md:justify-start md:text-right md:ml-6">
        <div className="text-2xl md:text-3xl font-bold text-gray-900 mb-0 md:mb-2">
          {isFutureSession ? (
            session.price > 0 ? (
              <>{getCurrency(session.currency)}{session.price.toLocaleString('it-IT', { minimumFractionDigits: 0 })}</>
            ) : (
              <>Scopri</>
            )
          ) : (
